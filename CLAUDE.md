...
- `POST /api/generate` (no `requireAuth` — accepts both authed and anonymous via `readAuth`) — picks the allowance in order **Pro (unlimited) → free monthly (signed-in only) → credits**, returns `402` if none, then prompts Claude for a **raw JSON resume object** (large fixed schema). Strips ``` fences, `JSON.parse`s, increments `cvCount`, and decrements `credits` when a credit was used. The request's `field` selects a `PROFESSIONS` entry (engineering/medical/legal/teaching/finance/business/design/sales/general) whose `guidance` + suggested `sections` are injected into the prompt to specialize the resume. The schema includes an optional `extraSections: [{ heading, items[] }]` the model fills with field-specific blocks (licenses, bar admissions, publications, portfolio, etc.), rendered in the CV's main column. The frontend separately maps `field` to the skills-section heading and to profession-aware input placeholders (`applyFieldHints`). An **optional `jobDescription`** (capped at 6000 chars) tailors the resume to a specific posting: when present, a `jdBlock` is injected into the prompt instructing the model to mirror the JD's exact keywords and reframe (never fabricate) the candidate's real experience to match. Available to all users, charged as a normal generation; the frontend exposes it as a collapsible "Tailor to a specific job" field (`#f-jd`, `toggleJD()`). No keyword/match panel is shown by design — since the app generates the resume, surfacing "missing keywords" would only indict its own output. The builder also pre-fills itself from URL query params via `applyUrlPrefill()` (run on load, after `restoreCVState`) — supported keys `role/field/name/email/phone/location/linkedin/portfolio/company/exp/jd`, all capped and set as plain text. This is the **funnel bridge**: an external job-board (separate repo) deep-links each posting into the builder with the job's `jd` preloaded, landing the visitor on a form already set to "Tailor to this job".
